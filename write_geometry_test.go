...
var polygon_geometry = []uint32{0x9, 0xf50, 0xda0, 0x22, 0x28, 0xf, 0x498, 0xd8, 0x317, 0x4d0, 0x1a7, 0x597, 0xf}
var multipolygon_geometry = []uint32{0x9, 0x9a0, 0xab0, 0x2a, 0x328, 0xa58, 0x5f0, 0x50f, 0x1f, 0x687, 0x5ff, 0x277, 0x2f7, 0x3b8, 0xf, 0x9, 0x958, 0x4e7, 0x22, 0xf, 0x31, 0x3e0, 0x2ca, 0x17, 0x628, 0x3b7, 0x8bf, 0xf}
...
func TestPolygonFloat(t *testing.T) {
	...
	for i := range cur.Geometry {
		if cur.Geometry[i] != polygon_geometry[i] {
			t.Errorf("Polygon Geometry Error %v %v",cur.Geometry[i],polygon_geometry[i])
		}
	}
}
